...
- **Length edits use simple time-stretch.** Extending a note (up to 4×) uses OLA time-stretch, so large factors can sound a little phasey and don't preserve transients; stretching is per-note and only along the note's own audio (it doesn't borrow surrounding material).
