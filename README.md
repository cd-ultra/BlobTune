...
| `js/fft.js` | **A dependency-free radix-2 FFT** (in-place Cooley–Tukey). Hand-written so BlobTune can do spectral analysis with no libraries or build step — used by onset detection today, and available for a spectrogram / polyphony later. |
