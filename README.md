...
- **Detection: YIN.** For each frame we compute the difference function, its cumulative-mean-normalized form, take the first dip below an absolute threshold, and parabolically interpolate for a sub-sample period. Clarity = `1 − d'(τ)`. Frames below a relative RMS floor are treated as unvoiced. Defaults: 2048-sample frames, 512-sample hop, voice-tuned range ~80–1000 Hz.
- **Octave-jump repair (for singing).** YIN on a real voice occasionally latches onto twice/half the true period for a few frames, reading as a sudden ±12-semitone leap and back. A post-process (`Pitch.octaveCorrect`) snaps each frame to the octave nearest the *median* of its voiced neighbours, so those transient errors are pulled back while a genuinely sustained octave change is kept (once enough frames sit at the new octave, the local median follows). On clean tones every frame already equals its neighbours' median, so this is a no-op.
- **Segmentation.** Split decisions run on a **median-smoothed** copy of the per-frame MIDI (removes single-frame jitter) compared against the note's **robust centre** — the median of a trailing window rather than a running mean that a vibrato would bias. A frame only starts a new note when the smoothed pitch sits more than ~1.1 semitones from that centre **and stays there** (hysteresis, ~3 frames), so a vibrato wobble that snaps back is reabsorbed instead of shattering one held note into many blobs. The note's reported pitch is still the median of the *raw* frames, so accuracy is unchanged. Short (≤2-frame) unvoiced gaps are still bridged (a real breath/silence between notes exceeds this and separates them, including same-pitch repeats); notes shorter than 4 frames are discarded.
...
- **Singing support is heuristic.** Median smoothing + hysteresis + octave repair make sustained sung notes (with vibrato) segment into stable blobs, but the segmentation cues are still pitch- and gap-based: two of the *same* pitch sung back-to-back are only split when there's an energy/breath dip between them (there is no dedicated onset/re-articulation detector), and heavy vibrato wider than the ~1.1-semitone tolerance, or fast legato runs of steps ≤1 semitone with no gap, can still mis-segment. These improvements were validated on a **synthetic** singing signal (vibrato, breath gaps, formant-like harmonics, noise, a same-pitch repeat and an octave move), not on a range of real microphone recordings.
